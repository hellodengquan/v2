...
#### 2.2.2 查询构建器 API 层面是否开放 ts_rank_cd 与归一化参数

**结论：完全不开放，硬编码死。** 代码层面三重锁定：

**锁定一：WithSearchQuery 方法签名无排名函数参数**

```go
// entry_query_builder.go:46
func (e *EntryQueryBuilder) WithSearchQuery(query string) *EntryQueryBuilder
//                                         ↑ 只有 query 一个参数
//                                            没有 rankingMode / normalization 选项
```

**锁定二：ts_rank 函数名硬编码在 SQL 字符串中**

```go
// entry_query_builder.go:54-56 — 两处 websearch_to_tsquery 都搭配 ts_rank
e.sortExpressions = append(e.sortExpressions,
    fmt.Sprintf("ts_rank(document_vectors, websearch_to_tsquery($%d)) ...", nArgs),
//              ↑ 硬编码函数名，不可能通过参数切换 ts_rank_cd
)
```

**锁定三：EntryQueryBuilder 结构体无相关字段**

```go
// entry_query_builder.go:20-29 — 结构体没有 rankingMode / normalization 字段
type EntryQueryBuilder struct {
    ...
    excludeContent  bool
    // ↑ 没有 rankingFunction / normalizationOption / useCoverDensity 等字段
}
```

**归一化参数（normalization）同样硬编码**：

`ts_rank` / `ts_rank_cd` 函数签名均支持第三个可选整数参数 `normalization`（位掩码，控制是否按文档长度归一化），但 Miniflux 的调用：
```go
ts_rank(document_vectors, websearch_to_tsquery($%d))
//                                  ↑ 只有 2 个参数，不传第三个
//                                  → 等价于 normalization = 0（默认值，不归一化）
```

**归一化 = 0 的实际含义**：
- 完全忽略文档长度对得分的影响
- 200 字短文出现 2 次关键词 vs 20000 字长文出现 2 次关键词 → **得分相同**
- 长文因总词数多，出现搜索词的概率天然更高，因此不归一化会让长文在排名中占便宜（但时间衰减系数部分抵消了此效应，因为长文通常是旧条目）

**如果未来要扩展**，需要做三件事：
1. 在 `EntryQueryBuilder` 加 `searchRankingMode` / `searchNormalization` 字段
...
虽然 Miniflux 当前**未使用** `ts_rank_cd`，但理解两者的交互对评估"如果切换到 ts_rank_cd 会怎样"至关重要。

##### 2.2.3.1 tsquery 操作符优先级（从高到低）

PostgreSQL `tsquery` 内部操作符优先级由 parser 硬编码定义，与 websearch 语法转换后结果一一对应：

| 优先级 | 操作符 | 含义 | websearch 对应语法 |
|-------|--------|------|-------------------|
| **1（最高）** | `!` | NOT（否定） | `-word`（前缀减号） |
| **2** | `<->` / `<`N`>` | FOLLOWED BY（邻近 / 相距 N 词） | `"phrase"`（带引号短语） |
| **3** | `&` | AND（与） | 普通空格分隔的词 |
| **4（最低）** | `\|` | OR（或） | `OR` 关键字 |
...
```
用户输入:  postgresql "full text" search -nosql OR database
           ↓ websearch_to_tsquery 转换
'postgresql' & 'full' <-> 'text' & 'search' & !'nosql' | 'database'
           ↓ 按优先级加括号（等价展开）
( ( 'postgresql' & ( 'full' <-> 'text' ) & 'search' & !'nosql' ) | 'database' )
```

注意：`!` 优先级最高，所以 `!a & b` = `(!a) & b`，而不是 `!(a & b)`。

##### 2.2.3.2 ts_rank_cd 如何"消费"带操作符的 tsquery

`ts_rank_cd`（覆盖密度算法）的核心思想是：**在文档中找到包含所有查询词的最小区间（覆盖区间），区间越小、词越多 → 密度越高 → 得分越高**。不同操作符对密度计算的影响不同：

| 操作符 | 对 ts_rank_cd 的影响 | 举例 |
|--------|---------------------|------|
| `&` AND | 所有 AND 连接的词必须都出现在覆盖区间内，词越多、区间越近 → 得分越高 | `postgres & search & index` 三词紧邻得分 >> 三词分散各段 |
| `<->` FOLLOWED BY | `<->` 本身就是邻近约束，匹配处天然形成极小区间，**大幅提升 ts_rank_cd 得分** | `"full text"` 短语匹配处密度极高，几乎总是排在非短语匹配前面 |
| `\|` OR | ts_rank_cd 对 OR 分支**分别计算密度，取较大值**作为最终得分 | `postgres \| mysql` 取两者中密度较高的那个作为得分 |
| `!` NOT | 否定词**不参与密度计算**（它们不出现在匹配处），仅用于 WHERE 层过滤 | `postgres & !mysql` 只对 postgres 做密度计算，mysql 只是排除条件 |

**关键：短语匹配（`<->`）与 ts_rank_cd 的协同效应**

当用户输入带引号的短语（websearch 语法 `"full text"`），同时用 `ts_rank_cd` 排序时，会产生**双重强化**：

1. **匹配层（WHERE @@）**：`<->` 要求词必须相邻，缩小了候选集
2. **排序层（ts_rank_cd）**：相邻词形成极小覆盖区间，密度极高，得分远超非短语匹配

**可能的"冲突"现象（优先级错觉）**：

用户可能观察到的"操作符优先级冲突"实际是以下原因导致的**感知偏差**：

> 输入：`"fat rat" cat` — 预期短语 fat rat 优先级高，但实际排名被 `cat` 的高频出现拉低/拉高

**真相**：
- 查询解析结果：`'fat' <-> 'rat' & 'cat'`
- 因为 `<->` 优先级高于 `&`，所以是 `(fat <-> rat) AND cat`
- ts_rank_cd 计算的是"同时包含 fat-rat 短语 和 cat 词"的覆盖密度
- 如果 cat 在文档中大量出现但远离 fat-rat 短语，覆盖区间会被拉长 → 密度**降低**
- 如果 cat 紧邻 fat-rat 短语出现，覆盖区间紧凑 → 密度**升高**

**换句话说**：不是操作符优先级有冲突，而是 AND 连接的多个词/短语需要**共同决定**覆盖区间的大小，额外增加的 AND 词如果远离已有匹配，反而会**拉低** ts_rank_cd 得分（因为区间变大了）。这与 `ts_rank`（词频累加，多一个 AND 匹配词只会加分不会减分）形成鲜明对比。

**总结对照表（ts_rank vs ts_rank_cd 对多词 AND 的反应）**：

| 场景 | ts_rank（Miniflux 在用） | ts_rank_cd（未用） |
|------|------------------------|-------------------|
| 新增一个 AND 匹配词 | 得分**上升**（多了一次词频贡献） | 得分**可能下降**（覆盖区间被拉长，密度降低） |
| 词都聚在一起 | 得分 = 词频之和，与位置无关 | 得分**很高**（区间小，密度大） |
| 词分散在文档各处 | 得分 = 词频之和，与位置无关 | 得分**较低**（区间大，密度小） |
| 短语匹配（`<->`） | 得分与同频普通 AND 相同（不考虑位置） | 得分**显著升高**（短语就是最紧凑的覆盖） |
...
#### 4.4.3.1 中英混排超长正文的截断触发条件与命中召回深度分析

单纯说"50 万字节后截断"不够精确，**中英混排场景下截断的真实行为取决于截断点落在什么类型的字符上**，进而影响尾部词汇的命中召回率。

##### 真实触发条件（从代码逐行推导）

`truncateStringForTSVectorField(s string, maxSize int)` 的执行路径：

```go
if len(s) < maxSize {
    return s         // 路径 A: 不足上限，直接返回（不触发截断）
}
truncated := s[:maxSize-1]   // 路径 B: 先按字节切一刀（可能破坏 UTF-8 字符）
for i := len(truncated) - 1; i >= 0; i-- {
    if (truncated[i] & 0x80) == 0 {
        return truncated[:i+1]  // 路径 B1: 遇到 ASCII（0xxxxxxx），停在此
    }
    if (truncated[i] & 0xC0) == 0xC0 {
        return truncated[:i]    // 路径 B2: 遇到多字节起始（11xxxxxx），停在它之前
    }
}
return ""            // 路径 C: 极端 fallback（全是 continuation byte）
```

**中英混排的三种典型触发场景**：

| 场景 | 截断点附近文本 | 执行路径 | 截断效果 | 对命中的影响 |
|------|--------------|---------|---------|-------------|
| **场景 A（英文结尾）** | 末尾是英文单词 + 空格 + 中文 | 路径 **B1**（遇到 ASCII 停） | 在一个 ASCII 字符处截断，该 ASCII 字符本身完整保留 | 如果截断点落在英文单词**中间**，该单词前半部分被保留，后半部分丢失（例："document" 切成 "docume"） |
| **场景 B（中文结尾）** | 末尾是连续中文 | 路径 **B2**（遇到 11xxxxxx 字节停，丢弃该汉字） | 截断点前一个**完整汉字**的位置结束，不会保留半个汉字 | 最后一个完整汉字之后的所有内容（包括该汉字之后的词）全部丢失 |
| **场景 C（混合边界）** | 截断点恰好在中英交界处 | 可能 B1 也可能 B2，取决于边界对齐 | 行为与 A 或 B 相同 | 边界处的词可能被切割 |

##### 英文单词被切中段的命中影响

**这是中英混排下最"隐蔽"的召回损失——不是完全搜不到，而是单词搜不到但前缀可能意外匹配。**

示例：正文末尾有单词 `documentation`，截断点恰好从中间切开，保留 `docume`（前 6 个字母）。

| 搜索词 | 能否命中 | 原因 |
|-------|---------|------|
| `"documentation"` | ❌ 不能 | `to_tsvector` 索引的是 `docume`（残缺字符串），查询的 `documentation` 词干化后是 `document`，不匹配 |
| `"document"` | ❌ 不能 | 索引的是 `docume`，不是 `document`，词干也对不上 |
| `"docume"` | ✅ 能 | 恰好就是残缺字符串本身（但用户不可能搜这个） |
| `"docu:*"`（前缀搜索） | ✅ 能 | 前缀匹配（但 Miniflux 不用 `to_tsquery` + `:*` 语法） |

**注意**：Miniflux 使用 `websearch_to_tsquery`，它不支持 `:*` 前缀匹配语法（那是 `to_tsquery` 的特性），因此**被截断的英文单词永远无法通过正常搜索命中**。

##### 中文连续字符串被截断的命中影响

中文因为没有空格分词边界，截断行为相对"干净"——最后一个完整汉字之后的所有内容直接丢掉。但对搜索召回的影响取决于：

1. **被截断部分是否包含用户可能搜索的词**
   - 如果截断点在段落中间：后面的句子、术语、关键词全部丢失
   - 如果截断点恰好在段落末尾：损失相对较小（可能只是段尾不重要的修饰语）

2. **截断点附近的"半个词"问题**
   - 中文词是多字组合（如 "数据库" 由 3 个字组成）
   - 如果截断点把 "数据库" 切成了 "数据"（保留前两字），索引中会有 "数据" 但没有 "数据库"
   - 用户搜 "数据" → 能命中（恰好就是前两字）
   - 用户搜 "数据库" → 不能命中（缺第三个字）
   - 这是一种**部分召回**现象：短词可能命中，长词不行

##### 量化：中英混排下"有意义的可索引文本量"是多少？

用**字节数 ÷ 平均单字字节数**反推实际字符量：

| 文本类型 | 平均每字字节 | 50 万字节对应字符数 | 与纯英文对比 |
|---------|------------|-------------------|-------------|
| 纯英文 | 1.0（1 字节/字符） | ~500,000 字符 | 100% |
| 代码片段混排 | ~1.5（变量名+注释） | ~333,000 字符 | 67% |
| 技术博客中英混排 | ~2.0（中文句子 + 英文术语） | ~250,000 字符 | 50% |
| 普通中文文章 | ~3.0（几乎全中文） | ~166,667 汉字 | 33% |
| 纯中文生僻字 | ~4.0（四字节 UTF-8） | ~125,000 汉字 | 25% |

**关键结论**：
- **文章英文比例越高，可索引的字符数越多**（因为英文 1 字节/字符）
- **但英文单词被从中间切断的概率也越高**（因为英文有明确的词边界，截断点落在单词中间的概率高）
- 中文文章虽然可索引字符少，但每个汉字都是完整的（UTF-8 边界回退保证），不存在"半个汉字"的问题
- 对技术类 RSS（中英混排、代码多），实际可索引文本量大约在 25~33 万字之间，比纯中文的 16.6 万字多，但远低于纯英文的 50 万字符

##### 截断对召回率影响的真实触发阈值

用户能感知到"搜不到"的触发条件（同时满足）：

```
1. 正文总字节数 >= 500,000 字节         ← 触发截断
   AND
2. 被截断的尾部区域（500,000 字节之后）   ← 尾部有"值钱"的词
   包含用户可能搜索的有意义词汇
   AND
3. 这些词汇在文章前 50 万字节中没有出现过  ← 前面没重复，搜不到
```
...
| 内容类型 | 触发概率 | 原因 |
|---------|---------|------|
| 短评/资讯（< 5000 字） | 0% | 远不到 50 万字节 |
| 教程中篇（2~5 万字） | 0% | 中文 5 万字 ≈ 150KB < 500KB |
| 长篇连载/小说（> 20 万字） | **中高** | 尾部的人名、地名、关键情节可能只出现一次 |
| 技术文档镜像（> 10 万字 + 代码） | **中** | 代码+英文占字节少但字符多，尾部 API 列表可能被截断 |
| 抓取全文的长文（readability） | **高** | readability 抓取整站文章可能几十万字，尾部参考链接、附录常被截断 |
